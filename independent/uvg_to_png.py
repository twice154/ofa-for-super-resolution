...
def extract_frames(videos, inDir, outDir):
    ...
    for video in videos:
        ...
        # for UVG DATASET parsing below
        retn = os.system('ffmpeg -f rawvideo -framerate 120 -s 1920x1080 -pixel_format yuv420p -i {} {}/%04d.png'.format(os.path.join(inDir, video), os.path.join(outDir, os.path.splitext(video)[0])))
        if retn:
...
def create_clips(root, destination):
    ...
    for file in files:
        ...
        for imageCounter, image in enumerate(images):
            ...
            if (imageCounter % 17 == 0):
                if (imageCounter + 16 >= len(images)):
                    break
                folderCounter += 1
...
##########
이미지 120fps로 가져와서 비디오 120fps 만들기
ffmpeg -framerate 120 -i ./beauty_120fps/%04d.png -c:v libx264 -pix_fmt yuv420p -r 120 output.mp4

Bitrate Constraint 까지 주려면
ffmpeg -framerate 120 -i ./beauty_120fps/%04d.png -c:v libx264 -b:v 1M -pix_fmt yuv420p -r 120 beauty_1M.mp4