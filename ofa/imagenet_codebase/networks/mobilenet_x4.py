...
class MobileNetX4(MyNetwork):
    ...
    def __init__(self, enc_first_conv, enc_input_skip_conn, enc_final_conv_blocks,
                        blocks,
                        dec_first_conv, dec_input_skip_conn, dec_final_conv_blocks):
        super(MobileNetX4, self).__init__()

        self.enc_first_conv = enc_first_conv
        self.enc_input_skip_conn = enc_input_skip_conn
        self.enc_final_conv_blocks = nn.Modulelist(enc_final_conv_blocks)

        self.blocks = nn.Modulelist(blocks)

        self.dec_first_conv = dec_first_conv
        self.dec_input_skip_conn = dec_input_skip_conn
        self.dec_final_conv_blocks = nn.Modulelist(dec_final_conv_blocks)

    def forward(self, x):
        x = self.enc_first_conv(x)
        enc_skip = self.enc_input_skip_conn(x)
        for block in self.blocks[:5]:
            x = block(x)
        for i, block in enumerate(self.enc_final_conv_blocks):
            x = block(x)
            if i == 0:
                x += enc_skip

        x = self.dec_first_conv(x)
        dec_skip = self.dec_input_skip_conn(x)
        for block in self.blocks[5:]:
            x = block(x)
        for i, block in enumerate(self.dec_final_conv_blocks):
            x = block(x)
            if i == 0:
                x += dec_skip
